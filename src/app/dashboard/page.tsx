...
import { useCogneeRecommendations } from '@/hooks/useCogneeRecommendations';
...
function Page(): React.ReactElement {
  ...
  const { recommendations: cogneeRecommendations } = useCogneeRecommendations({
    ...
    enabled: !dataLoading && data.recommendations.length === 0 && Boolean(selectedBrandId),
  });

  const displayRecommendations = useMemo(() => {
    if (data.recommendations.length > 0) return data.recommendations;
    if (cogneeRecommendations.length > 0) return cogneeRecommendations;
    return recommendationsData;
  }, [data.recommendations, cogneeRecommendations]);
