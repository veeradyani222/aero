...
function ChatGPTMarkdownRenderer({ content }: { content: string }) {
  ...
  const renderMarkdown = (text: string) => {
    ...
    processed = processed.replace(/\*\(source:\s*([^)]+)\)\*/g, '<span class="inline-flex items-center px-2 py-1  text-xs font-medium bg-green-100 text-green-800 ml-2">ChatGPT Source: $1</span>');
    ...
    processed = processed.replace(/\[(\d+)\](?!\()/g, '<sup class="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-black bg-green-500  ml-1">$1</sup>');
      ...
      <div className="px-6 py-4 border-b border-white/10">
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-black">ChatGPT Response</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-1.5 h-1.5 bg-white/40"></div>
            <span className="text-xs font-bold uppercase tracking-widest text-black/40">Web Search Disabled</span>
          </div>
        </div>
      </div>
      <div className="p-6">
    ...
    <div className="bg-white/5 border border-white/10">
      <div className="px-6 py-4 border-b border-white/10">
        <div className="flex items-center space-x-3">
          <div className="w-1.5 h-1.5 bg-primary"></div>
          <span className="text-xs font-bold uppercase tracking-widest text-black">ChatGPT Web References ({links.length})</span>
        </div>
      </div>
      <div className="p-6">
        {links.length > 0 ? (
            ...
            {links.map((link, index) => (
              <div key={index} className="flex items-start space-x-3 p-4 bg-white/5 border border-white/10 hover:border-primary transition-all duration-300">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-black/40 mt-1 truncate">{link.url}</p>
                  {link.source && (
